 ...
 */
export async function connect(interaction: Discord.ChatInputCommandInteraction) {
  try {
    ...
    if (!verificationCode) {
      ...
      await handleResponse(
        ...
        `/github connect username:${githubUsername} verification_code:${newVerificationCode} use_oauth:false`,
        { ephemeral: true }
      )
      return
    }
